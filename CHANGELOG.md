...
- Forward the client's `interleaved-thinking-*` beta header (e.g. `interleaved-thinking-2025-05-14`) to the upstream on the native `/v1/messages` (and `/v1/messages/count_tokens`) route, extending the same allowlist used for `computer-use-*` tokens. Stripping the header was benign (requests still succeeded and thinking blocks were produced), but forwarding it keeps thinking-block placement between tool calls identical to a direct upstream connection. All other client `anthropic-beta` tokens are still stripped.

- Support server-side compaction on the native `/v1/messages` (and `/v1/messages/count_tokens`) route: when `context_management.edits` contains a `compact_*` edit type (e.g. `compact_20260112`), the proxy now auto-adds the `anthropic-beta: compact-2026-01-12` header the upstream requires. Previously the client's compaction beta header was stripped and the upstream rejected the edit type with `400`; the Copilot upstream supports compaction when the beta is present (verified live).
...
- Add `docs/upstream-messages-live-tests.zh-CN.md` — a live-test record (2026-07-21/22) of the Copilot upstream `/v1/messages` endpoint, cross-verified through the proxy and via a direct upstream connection: web_search / web_fetch / image-URL blocked at the gateway for all models (latest Anthropic tool versions included), structured outputs working on Anthropic- and Bedrock-routed models but blocked by a GCP org policy on Vertex-routed ones (`sonnet-4.5` / `opus-4.5`), real `max_tokens` caps (128K / 64K, higher than the `/models`-advertised values; 300K rejected), and a ~630-second SSE stream lifetime terminated by an HTTP/2 RST_STREAM with no terminal events — with per-model throughput numbers and client-side handling advice.
