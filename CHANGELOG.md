...
- Keep native `/v1/messages` streams alive during long thinking phases: when the upstream sends no bytes for `COPILOT2API_SSE_KEEPALIVE_SECONDS` (default `15`, set `0` to disable), the proxy injects an Anthropic `ping` event (`event: ping` / `data: {"type": "ping"}`) so idle SSE connections are not evicted by NATs, CDNs, gateways, or load balancers. Pings are only ever emitted on SSE event boundaries, so the forwarded stream stays byte-identical once they are removed, and conforming Anthropic clients discard them without touching the message accumulator.
- Bound native `/v1/messages` streams with an upstream-silence ceiling: if the upstream sends no bytes for `COPILOT2API_SSE_MAX_IDLE_SECONDS` (default `600`, set `0` to disable), the stream is aborted with a terminal Anthropic `error` event instead of being kept open indefinitely by keep-alive pings. This releases the client and upstream connections when the upstream wedges, and applies whether or not keep-alive is enabled.
- Send `X-Accel-Buffering: no` on every SSE response. nginx buffers proxied responses by default (`proxy_buffering on`), which held back incrementally flushed events — including keep-alive pings — until the buffer filled or the stream ended. Deployments behind nginx and compatible reverse proxies now stream without extra configuration; proxies that don't recognize the header ignore it.
...
- Send `anthropic-version: 2023-06-01` and the base `interleaved-thinking-2025-05-14` beta on native `/v1/messages` and `/v1/messages/count_tokens` upstream requests. Client `anthropic-beta` tokens remain filtered except for the narrow `computer-use-*` allowlist required by Anthropic Computer Use tool types; duplicate allowlisted tokens are removed. A `context-1m*` token is still consumed locally to select the `-1m` model variant rather than forwarded.
...
- Fix native `/v1/messages` streaming responses withholding the HTTP response headers until the first upstream byte arrives. The headers (including `Content-Type: text/event-stream`) are now flushed as soon as the upstream stream is established, so long thinking phases no longer trip client and intermediary response-header timeouts before any data is produced. This matches the behavior the OpenAI and Gemini streaming paths already had.
- Fix native `/v1/messages` streams continuing to drain the upstream response after the client has disconnected. The proxy now aborts as soon as the request context is cancelled (or a write fails), releasing the upstream connection instead of reading the remaining response — which could previously run for several more minutes on long streams.
- Fix `POST /v1/responses` returning `400 "Invalid JSON in request body"` for every model that does not natively support the Responses API upstream (for example `claude-*`, `gemini-*`, `kimi-k2.7-code`). The Responses→Chat Completions conversion path decoded `input` as an array only, so the OpenAI-documented string shorthand (`"input": "hello"`) failed to parse. `input` now accepts both a plain string and an array of input items.
- Fix `POST /v1/responses` dropping input items that omit the optional `type` field when the request is converted to Chat Completions, which surfaced as an upstream `400 "messages must be non-empty"`. Items carrying a `role` are now treated as messages, matching the Responses API where `type` defaults to `"message"`.
- Fix `GET /usage` for accounts using `direct` auth mode by querying `copilot_internal/user` with the raw GitHub OAuth token. The response now includes the account's Copilot plan, quota reset date, and `chat`, `completions`, and `premium_interactions` quota snapshots instead of failing through the exchange-only `copilot_internal/v2/token` path.
...
- Add `docs/copilot2api-issues-retrospective.html` — a 27-slide 16:9 HTML retrospective (same TD dossier style and navigation as the capability report) summarizing all issues tested and consulted between 2026-06-10 and 2026-08-13, organized as symptom → analysis → resolution: upstream capability gaps and the beta-header allowlist (re-tested 2026-08-04: URL-based images and PDFs are both rejected by the Copilot gateway in direct and proxy paths and must be sent as base64; fast mode flipped to supported on 2026-07-26 — the dedicated `claude-opus-4.8-fast` model ID returns `usage.speed: "fast"` with a measured 2.46× output speedup in both auth modes, while the official `speed` parameter + beta header remains silently ignored), HTTP 413 root cause on the Business endpoint, oversized-context behavior, thinking-signature loss on the `/chat/completions` conversion path, structured-outputs failures on the Vertex backend, token refresh and direct-token auth, SSE stream drops (linked to upstream claude-code #70017) plus the native `/v1/messages` SSE Ping implementation, its downstream-only boundary, and the conditions under which Claude Code 2.1.222+ can use those pings to suppress false `Check your network` idle-watchdog warnings, Client-ID-gated model visibility in OpenCode-style flows (use OpenCode's Client-ID to obtain a `gho_` token for the full model list), a live `/models` probe comparing the `ghu_` (exchange) vs `gho_` (direct) model lists, the requirement to send `X-Github-Api-Version: 2026-06-01` to see the 1M context window, a live invocation test (42 calls on 2026-07-26 across `/v1/messages`, `/chat/completions`, and `/responses`) showing shared models behave identically in both auth modes while mode-exclusive models are enforced at invoke time (400) and Claude visibility is geo-filtered by the client's egress IP (without a VPN all Claude models disappear from both lists and invocations return 400, while other vendors are unaffected; behind a VPN they all return), dedicated inventories for all empirically supported general Anthropic features and beta-header behavior, a live beta-header acceptance matrix (74 calls on 2026-07-26: the Copilot gateway is blocklist-based, the opposite of api.anthropic.com — only 5 tokens are rejected with 400 `unsupported beta header(s)` (`output-128k`, `files-api`, `mcp-client-2025-11-20`, `skills`, `advisor-tool`) while the other 31 including made-up tokens pass through, identically in both auth modes, though acceptance does not imply activation), a backend routing re-test (150 calls on 2026-08-02 in exchange mode: Google Vertex has disappeared from the Claude routing pool — 0 hits vs ~20% on 07-25 — routing is now statically pinned per model with newest-generation models on the Anthropic first-party API and previous-generation models on AWS Bedrock, and the structured-outputs failures from Issue 04 no longer reproduce: 50/50 pass), and prompt-cache hit mechanics — including a live `cache_control` probe (2026-08-14, direct to `api.enterprise.githubcopilot.com` on `claude-opus-4.8`) confirming block-level ephemeral write/read, `scope` rejection, and the 1024-token minimum, and finding two upstream shifts from earlier conclusions: top-level automatic `cache_control` is now accepted (previously 400) and 1h TTL now works without the `extended-cache-ttl-2025-04-11` beta header; a proxy re-test through copilot2api confirms 6 of the 7 cases are byte-for-byte identical to direct, the only divergence being the intentional `cache_control.scope` strip (400 direct → 200 cached through the proxy). Adds a beta-header functional re-test slide (2026-08-19, direct to `api.githubcopilot.com` on `claude-opus-4.8`): the 28 `anthropic-beta` tokens that Anthropic still documents as required were each probed three ways (header only, feature payload with the header, and the same payload without it), which separates gateway acceptance from actual activation. Only three are genuinely required and functional upstream — `computer-use-2025-11-24`, `context-management-2025-06-27`, and `compact-2026-01-12` (each 400 without the header, 200 with it) — exactly matching what the proxy already forwards or injects, so the allowlist needs no widening. Six more are accepted but inert (notably `extended-cache-ttl-2025-04-11`, where a fresh-cache-key reversed-order control shows the 1h TTL bucket is populated with or without the header, and `thinking-token-count-2026-05-13`, whose `thinking_tokens` field is already returned by default), four are accepted while unavailable through Copilot's current synchronous interface (`output-300k`, both `server-side-fallback` versions, and `computer-use-2025-01-24`), and the five-token blocklist is unchanged from 2026-07-26 — reinforcing that stripping unknown client betas is a necessary protection, since a blocklisted token such as `files-api-2025-04-14` fails the entire request with a 400. Adds a dedicated 300K-output boundary slide explaining that Anthropic exposes the feature only through the asynchronous Message Batches API (`/v1/messages/batches` + `output-300k-2026-03-24`), while synchronous `/v1/messages` and the Copilot upstream remain capped at 128K.
- Expand the retrospective from 27 to 31 slides with four opening operating-guide pages: non-patterned account naming; a source-aligned request-header comparison for OpenCode/Direct and VS Code/Exchange (including dynamic initiator, vision, editor, integration, and request-ID fields); an outbound `anthropic-beta` allowlist with the five-token Copilot gateway blocklist; and scenario-specific identity isolation plus session-sticky, health-aware account-pool routing.
- Remove Computer Use coverage from the retrospective presentation while retaining the remaining beta-header filtering, capability, test, and operational guidance.
- Add `docs/copilot2api-quick-start.html`, a standalone 7-slide operational guide published separately as `quick-start.html`; its third slide compares OpenCode/Direct and VS Code/Exchange authentication, from GitHub Device Flow through direct bearer use or short-lived Copilot-token exchange and refresh.
- Revise the opening authentication recommendation in both HTML guides to prefer the OpenCode Device Flow, direct GitHub OAuth bearer usage, and the matching `ProfileOpenCode` request profile.
- Retitle the standalone quick-start deck as "GitHub Model Access Guidelines" and align its cover summary and metadata with the seven-slide deployment and integration scope.
