...
| 45 | `prompt_cache_1024`  🆕 | ✅ | ✅ | ✅ | ✅ | 4.8 缓存下限降至 1024;实测 1362 token 命中(4.7 不缓存) |
...
- **`prompt_cache_1024`**:Copilot **支持** 4.8 的下限降级。实测 **1362 token**(介于 1024–2048)的系统前缀成功命中缓存(直连 `cache_creation=1362`,随后经代理 `cache_read=1362`),证明 4.8 的 1024 下限生效(该前缀在 4.7 的 2048 下限下不会缓存)。
...
| `prompt_cache_1024` | ~1.36k token 系统前缀 + `cache_control` | support(命中缓存) | cc=1362 / cr=1362 |
