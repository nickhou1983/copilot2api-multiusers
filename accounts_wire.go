...
func newAccountHandlers(id, apiKey, pool, tokenDirRaw, tokenDirAbs string, transport *http.Transport, rec *stats.Recorder) (*accounts.Account, error) {
...
func buildAccount(ctx context.Context, id, apiKey, pool, tokenDirRaw, tokenDirAbs string, transport *http.Transport, rec *stats.Recorder) (*accounts.Account, error) {
	acct, err := newAccountHandlers(id, apiKey, pool, tokenDirRaw, tokenDirAbs, transport, rec)
	if err != nil {
		return nil, err
	}
	if id != "" {
...
func buildRegistry(ctx context.Context, baseTokenDir string, transport *http.Transport) (*accounts.Registry, *accounts.Manager, *stats.Store, error) {
	...
	for i := range cfg.Accounts {
		ac := cfg.Accounts[i]
		acct, err := buildAccount(ctx, ac.ID, ac.APIKey, ac.Pool, ac.TokenDir, ac.ResolveTokenDir(baseTokenDir), transport, statsStore.Recorder(ac.ID))
		if err != nil {
			return nil, nil, nil, err
		}
	...
	factory := func(c accounts.AccountConfig) (*accounts.Account, error) {
		return newAccountHandlers(c.ID, c.APIKey, c.Pool, c.TokenDir, c.ResolveTokenDir(baseTokenDir), transport, statsStore.Recorder(c.ID))
	}
