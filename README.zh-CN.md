...
- `api_key` —— 客户端需提供的 Key，各账号之间必须唯一，**除非**这些账号组成一个账户池（参见 [账户池](#账户池)，池成员共享同一个 Key）。
- `token_dir` —— 该账号 `credentials.json` 的存放位置。相对路径基于基础 Token 目录解析；默认为 `id`。
- `pool` *(可选)* —— 将该账号与其他账号归入同一个 [账户池](#账户池)，共享一个 API Key。
...
**账户池**让单个 API Key 背后挂多个 GitHub 账号，从而分摊负载、规避单账号的限流与配额。给两个或更多账号设置**相同的 `api_key`** 和**相同的非空 `pool`** 名称即可：
...
上例中 `sk-team-...` 路由到由 `team-a` + `team-b` 组成的池，而 `sk-solo-...` 仍是普通的一对一账号。

- **轮询（Round-robin）**：对池 Key 的每个请求依次分发给下一个成员，负载均匀分摊。
- **自动故障转移**：若某成员在**响应字节尚未开始下发之前**返回可重试状态（`429` 限流、`402`/`403` 配额、或瞬时 `5xx`），代理会透明地在下一个成员上重试。客户端只会看到成功成员的响应（若全部失败则看到最后一个成员的错误）。
- **向后兼容**：没有 `pool` 的账号行为与之前完全一致（单成员池，无重试开销）。共享 `api_key` 但**没有**匹配的 `pool` 名称仍会被判定为重复 Key 而拒绝。
- 每个成员保留各自独立的凭据存储、Token、模型缓存与用量统计（按各自 `id` 记录）。

> 目前仅实现轮询这一种选择策略。故障转移适用于非流式错误，以及在首字节前就失败的流式响应；一旦 `200` 流已经开始，就无法再切换到其他成员。
