...
- 将复盘报告扩展为 32 页，新增四模型 Prompt Cache 命中率矩阵：Direct 模式下仅调用 native `/v1/messages`，204/204 请求全部成功；横向对比 Sonnet 5、Sonnet 4.6、Opus 4.8 与 Opus 5 的静态重放、多轮追加、thinking+tools 衰减、最小可缓存前缀、严格前缀失效、流式与非流式一致性，以及 5 分钟 / 1 小时 TTL 行为。
