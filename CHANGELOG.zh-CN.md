...
- 原生 `/v1/messages`（及 `/v1/messages/count_tokens`）的上游请求现在发送一对固定请求头（对齐 opencode CLI）：`anthropic-version: 2023-06-01` 与 `anthropic-beta: interleaved-thinking-2025-05-14`。此前代理不发送 `anthropic-version`，且 `anthropic-beta` 为动态拼装（自动注入 `context-management-2025-06-27` / `compact-2026-01-12`，并按白名单转发客户端的 `computer-use-*` / `interleaved-thinking-*` token）；上述逻辑全部被固定值取代。客户端的 `anthropic-beta` 头一律不再转发——`context-1m*` token 仍会在本地被消费用于切换 `-1m` 模型变体。注意：依赖 `computer-use-*` 工具型的请求可能被上游拒绝，因为对应的 beta token 不再发送。
...
- 新增 `docs/upstream-messages-live-tests.zh-CN.md` —— Copilot 上游 `/v1/messages` 实测记录（2026-07-21/22，07-25 复测），经代理与直连上游两条链路交叉验证：web_search / web_fetch / 图片 URL 在网关层对全部模型拦截（含 Anthropic 最新工具版本，并以 base64 图片与普通工具作对照证明为定向拦截）；structured outputs 在 Anthropic / Bedrock 落点可用、Vertex 落点被 GCP 组织策略拦截——模型与后端**不是固定映射**且随时间漂移，故避开特定模型并非有效规避手段（应对该组织策略 400 做自动重试）；`max_tokens` 真实上限 128K / 64K（高于 `/models` 宣告值，300K 被拒）；SSE 流约 630 秒生存期，以 HTTP/2 RST_STREAM 终止且无任何终止事件——附各模型吞吐实测与客户端处理建议。
...
- 新增 `docs/copilot2api-issues-retrospective.html` —— 20 页 16:9 HTML 复盘报告（与能力报告同款 TD 档案风格与翻页交互），汇总 2026-06-10 ~ 07-22 期间测试与咨询过的全部问题，按「现象 → 分析 → 解决方案」组织：上游能力缺口与 beta 头白名单（2026-08-04 复测确认 URL 图片与 URL PDF 在直连和代理两条链路均被 Copilot 网关拒绝，必须改用 base64；2026-07-26 复测 fast mode 已翻转为支持：专属模型 ID `claude-opus-4.8-fast` 返回 `usage.speed: "fast"`，双模式实测 2.46× 输出加速；官方 `speed` 参数 + beta 头仍被静默忽略）、Business 端点 413 根因、超长上下文行为、`/chat/completions` 转换路径的 thinking signature 丢失、Vertex 落点 structured outputs 失败、Token 刷新与直连认证、SSE 断流排查（关联上游 claude-code #70017）、OpenCode 类方案下模型清单由 Client-ID 决定（改用 OpenCode 的 Client-ID 申请 `gho_` token 拿到完整清单）、实测对比 `ghu_`（交换）与 `gho_`（直连）两种方式的 `/models` 清单差异、必须发送 `X-Github-Api-Version: 2026-06-01` 才能看到 1M 上下文窗口、双模式真实调用实测（2026-07-26 共 42 次调用，覆盖 `/v1/messages`、`/chat/completions`、`/responses`：共有模型两种模式调用结果完全对等，独占模型在调用时被网关强制 400，且 Claude 可见性按客户端出口 IP 地域过滤 —— 未挂 VPN 时 Claude 全线从清单消失、调用 400，其余厂商不受影响，挂 VPN 后全部回归）、代理 Header 过滤（客户端 header 从不复制到上游 —— 认证头整套替换、客户端 `anthropic-beta` 一律不转发；代理固定发送 `anthropic-version: 2023-06-01` + `anthropic-beta: interleaved-thinking-2025-05-14` 基线，body 含 `context_management` 时成对追加 `context-management` + `compact` beta，`context-1m*` 被消费为切换 `-1m` 模型变体，请求体字节级透传仅改模型别名并删 `cache_control.scope` —— 2026-07-27 回显服务器探针实测验证）、beta 头接受度实测（2026-07-26 共 74 次调用：Copilot 网关为黑名单制，与 api.anthropic.com 正好相反 —— 仅 5 个 token 被 400 `unsupported beta header(s)` 拒绝（`output-128k`、`files-api`、`mcp-client-2025-11-20`、`skills`、`advisor-tool`），其余 31 个含乱造 token 全部放行，双模式逐 token 一致，但放行不等于生效）、后端路由复测（2026-08-02 Exchange 模式共 150 次调用：Google Vertex 已从 Claude 路由池消失 —— 0 命中，对比 07-25 约 20% —— 路由由动态漂移变为按模型静态绑定，最新代模型落 Anthropic 第一方、上一代主力落 AWS Bedrock，Issue 04 的 structured outputs 失败不再复现：50/50 全过）、缓存命中机制。
